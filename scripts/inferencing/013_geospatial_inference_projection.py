...
def herdnet_geospatial_inference(cfg: DictConfig,
    ...
    if plain_inference:
        ...
        test_dataset = animaloc.datasets.__dict__[cfg.datasets.name](
            csv_file = test_df,
            root_dir = images_dir,
            albu_transforms = [A.Normalize(cfg.datasets.mean, cfg.dataset.std)],
            end_transforms = [DownSample(down_ratio=down_ratio, anno_type=cfg.dataset.anno_type)]
            )

    else:
        test_df = pandas.read_csv(cfg.datasets.csv_file)
        _set_species_labels(cls_dict, df=test_df)

        test_dataset = animaloc.datasets.__dict__[cfg.datasets.name](
            csv_file = test_df,
            root_dir = images_dir,
            albu_transforms = [A.Normalize(cfg.datasets.mean, cfg.datasetsin.std)],
            end_transforms = [DownSample(down_ratio=down_ratio, anno_type=cfg.dataset.anno_type)]
            )
