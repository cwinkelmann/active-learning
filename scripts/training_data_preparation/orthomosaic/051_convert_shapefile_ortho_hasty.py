"""
Convert a shapefile and orthomosaic to image tiles and convert the annotations .
...
def main(train_orthophoto, gdf_train, tile_output_dir, jpg_tile_output_dir,
         hA_reference: HastyAnnotationV2 = None,
         dataset_name:str = "default",
         tile_x_size=1500, tile_y_size=1500,
         visualise_crops=False, vis_output_dir: Path = None) -> HastyAnnotationV2:
    ...
    gdf_grid = grid_manager.create_regular_grid(x_size=tile_x_size, y_size=tile_y_size, overlap_ratio=0.0)
    ...
    for tile_name, gdf_group in gdf_sliced_points.groupby(by="tile_name"):
        ...
        if visualise_crops:
            ...
            ax_s = visualise_image(image_path=jpg_tile_output_dir / tile_name, show=False,
                                   title=f"Visualisation of {len(gdf_herdnet)} labels in {tile_name}")
            visualise_polygons(
                points=[shapely.Point(x, y) for x, y in zip(gdf_herdnet.local_pixel_x, gdf_herdnet.local_pixel_y)],
                labels=gdf_herdnet["species"], ax=ax_s, show=False, linewidth=6, markersize=10,
...
if __name__ == "__main__":
    ...
    dataset_name = "body_hq"
    hasty_reference_path = Path("/Users/christian/data/training_data/2025_09_19_orthomosaic_data/fernandina_s_correction_hasty_corrected_1.json")
    # orthophoto path
    train_orthophoto_path = Path(
    ...
    hA_reference = HastyAnnotationV2.from_file(hasty_reference_path)

    hA_updated = main(train_orthophoto_path, gdf_train, tile_output_dir, jpg_tile_output_dir,
         hA_reference, dataset_name = dataset_name,
             tile_x_size=1500, tile_y_size=1500)


    hA_updated_flat = hA_updated.get_flat_df()
