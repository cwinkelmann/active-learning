...
# 1. Function to read images from a folder
def read_images_from_folder(folder_path, max_images=None, allowed_extensions=('.jpg', '.jpeg', '.png')):
    """
    Read images from a folder path.

    Args:
        folder_path (str): Path to the folder containing images
        max_images (int, optional): Maximum number of images to read. Default is None (all images).
        allowed_extensions (tuple, optional): File extensions to consider. Default is ('.jpg', '.jpeg', '.png').
...
# 2. Functions to calculate different quality metrics on image regions

def calculate_laplacian_variance(region):
    """
    Calculate Laplacian variance (measure of focus) for an image region.
    ...
    # Return the variance (measure of focus/sharpness)
    return laplacian.var()


def calculate_gradient_magnitude(region):
    """
    Calculate average gradient magnitude for an image region.

    Args:
        region (numpy.ndarray): Image region

    Returns:
        float: Average gradient magnitude
    """
    # Convert to grayscale if needed
    if len(region.shape) > 2:
        gray = cv2.cvtColor(region, cv2.COLOR_RGB2GRAY)
...
def calculate_local_contrast(region):
    """
    Calculate local contrast for an image region.
    ...
    # Return the standard deviation (measure of contrast)
    return np.std(gray)


def calculate_tenengrad(region):
    """
    Calculate Tenengrad value (measure of focus based on gradient) for an image region.

    Args:
        region (numpy.ndarray): Image region

    Returns:
        float: Tenengrad value
    """
    # Convert to grayscale if needed
    if len(region.shape) > 2:
        gray = cv2.cvtColor(region, cv2.COLOR_RGB2GRAY)
    ...
    # Calculate Tenengrad value
    return np.mean(gradient_x ** 2 + gradient_y ** 2)
...
def process_images(images: typing.List[Path], metrics, grid_size=(10, 10), max_workers=None, output_dir:Path=None):
    """
    ...
    process_args = [(image_path, grid_size, metrics) for image_path in images]
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing images"):
            try:
                ...
                # Optionally log which image caused the error
                # Continue with the next image


    metric_dumps = []
    for image_id, result in results:
        metric_dump = {"image_id": image_id, "grid_size": grid_size}
        for metric_name in metrics.keys():

            metric_result = result[metric_name].reshape(grid_size[0] * grid_size[1])
            metric_dump[metric_name] = metric_result

        metric_dumps.append(metric_dump)

    pd.DataFrame(metric_dumps).to_parquet(output_dir / "image_quality.parquet")
    pd.DataFrame(metric_dumps).to_csv(output_dir / "image_quality.csv")

    # TODO do not aggregate here anymore
    # Aggregate results
    for image_id, result in results:
        for metric_name in metrics.keys():
            # Add to aggregated results
            aggregated_results[metric_name] += result[metric_name]
...
def analyze_image_quality(folder_path, metrics, grid_size=(10, 10), max_images=None, output_dir=None):
    """
    ...
    # TODO make a generator out of this
    # 1. Read images
    images = read_images_from_folder(folder_path, max_images)

    if len(images) == 0:
        raise ValueError(f"No valid images found in {folder_path}")
    logger.info(f"Found {len(images)} images in {folder_path}")
    # 2. Process images
    aggregated_results = process_images(images, metrics=metrics, grid_size=grid_size, max_workers=6, output_dir=output_dir)
