...
def plot_orthomomsaic_training_data_zoom(group_extent, group_gdf, gdf_group_annotations, group_id, vis_config,
                                    gpkg_path="/Volumes/2TB/SamplingIssues/sampling_issues.gpkg",
                                    output_dir=None):
    """
    Plot orthomosaic training data for a specific group showing:
    ...
    Parameters:
    - vis_config['zoom']: float, zoom level (0.5 = zoom out, 1.0 = normal, 2.0 = zoom in)
    """

    epsg = vis_config["epsg"]
    inset_size = vis_config.get("inset_size", 3.0)  # Default to 1.0 if not specified
    ...
    # Set extent to group bounds with zoom-adjusted padding
    base_padding = max(group_bounds[2] - group_bounds[0], group_bounds[3] - group_bounds[1]) * 0.1
    # Inverse relationship: smaller zoom = more padding (zoom out), larger zoom = less padding (zoom in)
    adjusted_padding = base_padding / 1

    ax.set_xlim(group_bounds[0] - adjusted_padding, group_bounds[2] + adjusted_padding)
    ax.set_ylim(group_bounds[1] - adjusted_padding, group_bounds[3] + adjusted_padding)
    ...
    # Add inset map showing location in Galápagos
    iax = inset_map(ax, location=vis_config["inset_map_location"], size=inset_size, pad=0.1, xticks=[], yticks=[])
    ...
    # Add scalebar to inset map
    inset_bounds = islands_wm.total_bounds
    inset_width_km = (inset_bounds[2] - inset_bounds[0]) / 1000  # Convert to km
    # Choose appropriate scalebar length based on inset map extent
    if inset_width_km > 100:
        scalebar_length_km = 50
    elif inset_width_km > 50:
        scalebar_length_km = 25
    elif inset_width_km > 30:
        scalebar_length_km = 20
    else:
        scalebar_length_km = 5

    # Position scalebar in lower left of inset
    scalebar_x = inset_bounds[0] + (inset_bounds[2] - inset_bounds[0]) * 0.05
    scalebar_y = inset_bounds[1] + (inset_bounds[3] - inset_bounds[1]) * 0.1

    # draw_accurate_scalebar(iax, islands_wm,
    #                        location=(scalebar_x, scalebar_y),
    ...
    # # Add scale bar
    # draw_accurate_scalebar(ax, group_gdf_wm,
    #                        location=(group_bounds[0] + adjusted_padding / 2, group_bounds[1] + adjusted_padding / 2),
    #                        length_km=min(10, (group_bounds[2] - group_bounds[0]) / 1000 / 3),  # Adaptive scale
    #                        segments=2,
    #                        height=max(1000, adjusted_padding / 10))
