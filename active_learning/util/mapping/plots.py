...
from active_learning.util.mapping.helper import get_geographic_ticks, format_lat_lon
...
def plot_orthomomsaic_training_data(group_extent, group_gdf, gdf_group_annotations, group_id,
    ...
    group_gdf_wm = group_gdf.to_crs(epsg=web_mercator_projection_epsg)
    if not gdf_group_annotations.empty:
        annotations_wm = gdf_group_annotations.to_crs(epsg=web_mercator_projection_epsg)
    else:
    ...
    group_gdf_wm.plot(ax=ax, alpha=0.6, edgecolor='red', facecolor='lightblue', linewidth=2)
    ...
    x_ticks_proj, x_ticks_geo, y_ticks_proj, y_ticks_geo = get_geographic_ticks(
        ax, epsg_from=web_mercator_projection_epsg, epsg_to=4326)
    ...
    iax = inset_map(ax, location="upper right", size=3, pad=0.1, xticks=[], yticks=[])
    ...
    indicate_detail(iax, ax, web_mercator_projection_epsg, web_mercator_projection_epsg)
    ...
    if not annotations_wm.empty:
        legend_elements.append(
            Line2D([0], [0], marker='o', color='w', markerfacecolor='red',
                   markersize=8, label=f'Iguana annotations ({num_annotations})')
        )
...
Missions: {', '.join(group_gdf['Orthophoto/Panorama name'].unique())}"""
    ...
    add_text_box(ax, info_text, xy=(0.05, 0.98), fontsize=9, alpha=0.8, pad=0.5)

    plt.tight_layout()

    # Save if output directory provided
    if output_dir:
        output_path = Path(output_dir) / f"training_data_group_{group_id:03d}.png"
